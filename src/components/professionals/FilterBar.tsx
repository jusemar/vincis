          ...
          <div className="pt-4 mt-4 border-t border-border">
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  {specialties[filters.profession as keyof typeof specialties]?.map((spec) => (
      ...
      {showMobileFilters && (
        ...
        >
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-2 block">Especialidade</label>
              <select
                value={filters.specialty}
                onChange={(e) => handleChange('specialty', e.target.value)}
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-2 block">Avaliação Mínima</label>
              <select
                value={filters.minRating}
                onChange={(e) => handleChange('minRating', Number(e.target.value))}
                className="w-full px-4 py-3 rounded-xl bg-muted border border-border text-foreground"
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-2 block">Disponibilidade</label>
              <select
                value={filters.availability}
                onChange={(e) => handleChange('availability', e.target.value)}
                className="w-full px-4 py-3 rounded-xl bg-muted border border-border text-foreground"
        ...
        >
          {filters.profession !== 'all' && (
            <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm flex items-center gap-2 border border-primary/20">
              ...
              <button onClick={() => handleChange('profession', 'all')} className="hover:text-primary/70">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {filters.specialty !== 'Todas as Especialidades' && (
            <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm flex items-center gap-2 border border-primary/20">
              {filters.specialty}
              <button onClick={() => handleChange('specialty', 'Todas as Especialidades')} className="hover:text-primary/70">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {filters.location !== 'Todas as Localizações' && (
            <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm flex items-center gap-2 border border-primary/20">
              {filters.location}
              <button onClick={() => handleChange('location', 'Todas as Localizações')} className="hover:text-primary/70">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {filters.minRating > 0 && (
            <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm flex items-center gap-2 border border-primary/20">
              ...
              <button onClick={() => handleChange('minRating', 0)} className="hover:text-primary/70">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {filters.availability !== 'all' && (
            <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm flex items-center gap-2 border border-primary/20">
              {filters.availability === 'available' ? 'Disponíveis' : 'Indisponíveis'}
              <button onClick={() => handleChange('availability', 'all')} className="hover:text-primary/70">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
